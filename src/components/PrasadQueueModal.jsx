            ...
            <div className="space-y-4">
              ...
              {activeToken ? (
                <div className="space-y-4">
                  ...
                  {isNearingTurn && (
                    <div className="bg-gradient-to-r from-amber-600 via-gold to-amber-500 text-indigo-dark p-3.5 rounded-2xl border border-gold shadow-goldGlow animate-bounce text-xs font-bold flex items-center gap-2">
                      ...
                      <div>
                        <p className="font-extrabold uppercase tracking-wide">{t.turnComing}</p>
                        <p className="text-[11px] font-semibold text-indigo-dark/90">{t.turnComingDesc}</p>
                      </div>
                    </div>
                  )}

                  {isMyTurnServed && activeToken.status !== 'served' && (
                    <div className="bg-gradient-to-r from-emerald-600 to-teal-700 text-white p-3.5 rounded-2xl border-2 border-gold shadow-lg text-xs font-bold flex items-center gap-2 animate-pulse">
                      ...
                      <div>
                        <p className="font-extrabold text-gold uppercase tracking-wide">{t.turnNow}</p>
                        <p className="text-[11px] font-medium text-emerald-100">{t.turnNowDesc}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {activeToken.status === 'served' && (
                    <div className="bg-gradient-to-r from-gold/20 via-amber-500/15 to-gold/20 text-maroon p-3.5 rounded-2xl border-2 border-gold shadow-sm text-xs font-bold flex items-center gap-2">
                      ...
                      <div>
                        <p className="font-black text-maroon uppercase tracking-wide">{t.servedTitle}</p>
                        <p className="text-[11px] font-semibold text-gray-700">{t.servedDesc}</p>
                      </div>
                    </div>
                  )}

                  {/* Digital Scannable Token Pass Card */}
                  <div className="bg-gradient-to-b from-[#FAF7F2] to-white p-5 rounded-3xl border-2 border-gold shadow-warm text-center space-y-4 relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between">
                      <span className={`text-[10px] font-extrabold uppercase px-3 py-1 rounded-full flex items-center gap-1.5 shadow-2xs ${
                        activeToken.status === 'served'
                    ...
                    <div className="bg-gradient-to-r from-maroon to-[#5A141E] text-white py-3 px-4 rounded-2xl shadow-md border border-gold/40 space-y-0.5">
                      ...
                      <p className="text-4xl font-black font-mono tracking-widest text-gold drop-shadow-sm">
                        #{activeToken.token_number}
                      </p>
                    </div>
                    ...
                    <div className="bg-white p-3 rounded-2xl inline-block shadow-md border-2 border-gold mx-auto">
                      {qrDataUrl ? (
                        <img src={qrDataUrl} alt="Signed Prasad Token QR" className="w-48 h-48 mx-auto object-contain" />
                      ) : (
                    ...
                    {/* Live Progress Bar */}
                    <div className="space-y-1.5 text-left bg-amber-50/70 p-3 rounded-xl border border-amber-200">
                      <div className="flex justify-between text-[11px] font-bold text-gray-700">
                        <span>Serving #{counterStatus.current_serving_token}</span>
                        <span>Your Token #{activeToken.token_number}</span>
                      </div>
                      <div className="w-full h-3 bg-gray-200 rounded-full overflow-hidden p-0.5 border border-gold/30">
                        <div 
                          ...
                          style={{ width: `${isMyTurnServed ? 100 : Math.max(10, Math.min(95, 100 - (activeToken.token_number - counterStatus.current_serving_token) * 10))}%` }}
                        ></div>
                      </div>
                      <div className="flex justify-between text-xs font-bold pt-0.5">
                        <span className="text-gray-500">{t.estimatedWait}</span>
                        <span className="font-extrabold text-maroon font-mono">
                    ...
                    <div className="bg-white p-3 rounded-xl border border-gray-200 text-xs text-gray-800 space-y-1.5 text-left shadow-2xs">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-extrabold text-maroon">{activeToken.headcount || 2} {activeToken.headcount === 1 ? t.person : t.people}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500 font-semibold">Type:</span>
                        <span className="font-bold text-indigo-dark">{activeToken.prasad_label || t.freeThali}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500 font-semibold">Dining Location:</span>
                        <span className="font-bold text-gray-700">{activeToken.dining_hall || t.hallMain}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500 font-semibold">{t.issuedAt}:</span>
                ...
                <div className="text-center py-8 space-y-3 bg-[#FAF7F2] p-6 rounded-3xl border border-gray-200">
                  <div className="w-14 h-14 rounded-full bg-amber-100 text-maroon flex items-center justify-center mx-auto text-2xl font-bold">
                    🍲
                  </div>
