    ...
    <header className="relative bg-darkWarm.bg/95 backdrop-blur-2xl text-white border-b border-gold/30 sticky top-0 z-50 shadow-temple pt-[max(env(safe-area-inset-top,0px),0.5rem)] pb-1.5 px-3 sm:px-6">
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 sm:gap-4">
        ...
        >
          ...
          <div className="relative w-9 h-9 sm:w-11 sm:h-11 rounded-full p-[2px] bg-gradient-to-tr from-gold via-amber-200 to-amber-500 shadow-[0_0_14px_rgba(235,178,57,0.35)] group-hover:scale-105 transition-all overflow-hidden flex items-center justify-center bg-white shrink-0">
            <img 
          ...
          <div className="flex flex-col justify-center">
            <h1 className="text-base sm:text-lg md:text-xl font-black tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-white via-amber-100 to-gold font-heading leading-tight whitespace-nowrap drop-shadow-xs">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-1 sm:gap-1.5 text-[11px] sm:text-xs font-black px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-white/10 hover:bg-gold hover:text-indigo-dark text-amber-100 border border-gold/35 hover:border-gold transition-all shadow-sm active:scale-95 cursor-pointer backdrop-blur-md font-heading group"
            title="Switch Language (English / हिन्दी / ગુજરાતી)"
          >
            <Globe className="w-3.5 h-3.5 text-gold group-hover:text-indigo-dark transition-transform duration-300 group-hover:rotate-45 shrink-0" />
            <span className="tracking-wide">
              {currentLanguage === 'en' ? 'EN' : currentLanguage === 'hi' ? 'हिन्दी' : 'ગુજરાતી'}
            </span>
          </button>
          ...
          {currentUser?.role === 'admin' ? (
            ...
            <div className="bg-indigo-card/90 backdrop-blur-md p-0.5 sm:p-1 rounded-xl border border-gold/30 flex items-center gap-0.5 sm:gap-1 text-[11px] sm:text-xs font-bold shadow-inner">
              <button
                onClick={() => navigate('/home')}
            ...
            /* Normal Pilgrim Badge */
            <div className="flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs font-black px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-gradient-to-r from-amber-500/20 via-gold/25 to-amber-500/15 border border-gold/50 text-gold font-heading tracking-wide shadow-sm backdrop-blur-md select-none transition-all">
